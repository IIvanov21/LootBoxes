...
## Development
### Front End Design
The Front End application can be accessed by users using the NGINX VM's IP address on port 80. As described above in section Interaction Diagram the user connects to the NGINX which automatically redirects them to a VM machine in the swarm. When the user connects to the application they get presented by an interface designed with HTML(Jinja2) for the layout. The interface showcases all the information generated by the API back-end services, including History which is pulled out the database created with MySQL.
<br>
